...
function getServerActionOperUrl(doWhat){
	//当前应用服务器的ip
	//return "http://file.3d-focus.com/crs/i1/" + doWhat + "?1=1";
...
function veUploadify(initJsonValue, uploadFileId){
	var jsonObj = {};
	jsonObj["uploader"] = "/script/uploadFile/uploadify.swf";
	jsonObj["script"] = "";
	jsonObj["cancelImg"] = "/script/uploadFile/cancel.png";
	//选择文件服务器
	...
	try {
		...
		if(filedId){
			...
			if(!filedValue || parseInt(filedValue) <= 0){
				...
				if(imgObj){
					var defaultImag = "/script/uploadFile/j_004.png";
